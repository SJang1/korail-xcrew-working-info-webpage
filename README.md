...
이 프로젝트는 별도의 관리자 시스템을 포함하고 있습니다. 관리자는 사용자의 일정 정보를 조회하고, 사용자 정보를 수정하거나 비밀번호를 강제 재설정할 수 있습니다.

### 1. 초기 관리자 계정 생성

데이터베이스 마이그레이션을 실행하면 기본 관리자 계정이 생성됩니다 (`migrations/0005_create_admins_table.sql`).

- **기본 아이디:** `admin`
- **기본 비밀번호:** `admin123`

**주의:** 배포 후 보안을 위해 즉시 비밀번호를 변경하십시오.

### 2. 관리자 패널 접속

웹 브라우저에서 `/adm/login` 경로로 접속하여 관리자 로그인을 할 수 있습니다.

### 3. 주요 기능

- **대시보드 (`/adm/dashboard`)**: 전체 사용자 목록을 조회하고 관리할 수 있습니다.
- **사용자 상세 정보 조회**: 특정 사용자의 월간 일정 및 일일 근무(Dia) 정보를 조회할 수 있습니다.
- **사용자 관리**: 사용자 상세 페이지에서 "Manage User" 버튼을 통해 이름을 변경하거나 비밀번호를 강제 재설정할 수 있습니다.
- **설정 (`/adm/settings`)**:
    - 본인의 관리자 비밀번호 변경
    - 새로운 관리자 계정 생성
    - 기존 관리자 삭제 (본인 제외)
...
관리자 시스템은 일반 사용자와 분리된 인증 체계를 사용합니다. 관리자 토큰은 별도의 쿠키 및 세션 저장소(KV) 접두사(`admin:`)를 사용하여 관리됩니다.
